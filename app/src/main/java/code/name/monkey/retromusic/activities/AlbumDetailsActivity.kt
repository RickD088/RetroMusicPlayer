...
class AlbumDetailsActivity : AbsSlidingMusicPanelActivity(), AlbumDetailsView, CabHolder {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        artistImage.setOnClickListener {
            ...
            NavigationUtil.goToArtistOptions(this, album.artistId, commonData, artistPairs)
        }
        playAction.apply {
            setOnClickListener {
                if (commonData.localAlbum()) {
                    MusicPlayerRemote.openQueue(
                            ...
                            commonData.getLocalAlbum().songs!!.toCommonData(),
                            0,
                            true
                    )
                } else if (commonData.cloudPlayListSong()) {
                    MusicPlayerRemote.openQueue(
                            this@AlbumDetailsActivity,
                            commonData.getCloudPlaylistSongs().songs!!.toCommonData(),
                            0,
                            true
                    )
                }
    ...
    override fun album(commonData: CommonData) {
        this.commonData = commonData
        if (commonData.localAlbum()) {
            val album = commonData.getLocalAlbum()
            ...
            loadAlbumCover()
            simpleSongAdapter.swapDataSet(album.songs.toCommonData())
            albumDetailsPresenter.loadMore(album.artistId)
            albumDetailsPresenter.aboutAlbum(album.artistName!!, album.title!!)
        } else if (commonData.cloudPlayList()) {
            val playlist = commonData.getCloudPlaylistBean()
            ...
            val playlist = commonData.getCloudPlaylistSongs()
            if (playlist.songs.isNotEmpty()) {
                simpleSongAdapter.swapDataSet(playlist.songs.toCommonData())
                albumText.text = String.format(
                        "%s • %s",
                        intent.extras?.getString(EXTRA_ALBUM_NAME),
                        MusicUtil.getReadableDurationString(MusicUtil.getTotalCurSongsDuration(playlist.songs))
                )
            }
        }
    }
...
//    }
    ...
    private fun reload() {
        if (intent.extras!!.containsKey(EXTRA_ALBUM_ID)) {
            intent.extras?.getInt(EXTRA_ALBUM_ID)?.let {
                intent.extras?.getBoolean(EXTRA_TYPE_LOCAL)?.let { local ->
                    albumDetailsPresenter.loadAlbum(it, local)
    ...
    companion object {
        ...
        const val EXTRA_ALBUM_NAME = "extra_playlist_name"
