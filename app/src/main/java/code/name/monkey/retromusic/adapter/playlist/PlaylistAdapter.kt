...
class PlaylistAdapter(
    ...
    private fun getIconRes(playlist: Playlist): Drawable {
        return when (playlist.id) {
            Constants.UI_FAVORITE_PLAYLIST_ID ->
                TintHelper.createTintedDrawable(
                activity,
                R.drawable.ic_favorite_white_24dp,
                ThemeStore.accentColor(activity)
            )
            Constants.UI_DOWNLOAD_PLAYLIST_ID ->
                TintHelper.createTintedDrawable(
                activity,
    ...
    private fun getSongList(playlists: List<Playlist>): ArrayList<CommonData> {
        val songs = ArrayList<CommonData>()
        for (playlist in playlists) {
            if (playlist is AbsCustomPlaylist) {
                songs.addAll(playlist.getSongs(activity))
            } else if (playlist.id == Constants.UI_DOWNLOAD_PLAYLIST_ID) {
                // abram 伪装成播放列表的下载项，需要特别填充数据
                songs.addAll(LastAddedSongsLoader.getDownloadSongs(activity))
            } else {
    ...
    private fun getSongs(playlist: Playlist): ArrayList<CommonData> {
        val songs = ArrayList<CommonData>()
        if (playlist is AbsSmartPlaylist) {
            songs.addAll(playlist.getSongs(activity))
        } else if (playlist.id == Constants.UI_DOWNLOAD_PLAYLIST_ID) {
        ...
        init {
            ...
            menu?.setOnClickListener { view ->
                ...
                if (playlist.id == Constants.UI_DOWNLOAD_PLAYLIST_ID) {
                    popupMenu.menu.findItem(R.id.action_delete_playlist).isVisible = false
                    popupMenu.menu.findItem(R.id.action_rename_playlist).isVisible = false
                    popupMenu.menu.findItem(R.id.action_save_playlist).isVisible = false
                }
                if (playlist.id == Constants.UI_FAVORITE_PLAYLIST_ID) {
                    popupMenu.menu.findItem(R.id.action_delete_playlist).isVisible = false
                    popupMenu.menu.findItem(R.id.action_rename_playlist).isVisible = false
                }
