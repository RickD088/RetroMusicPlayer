...
import code.name.monkey.retromusic.fragments.RewardFragment;
...
public class MainActivity extends AbsSlidingMusicPanelActivity
    ...
    private ImageView collector_diamond;
    @Inject
    ...
    public boolean handleBackPress() {
        ...
        // 如果不在主页，则 back 键先回到主页
        if (!(currentFragment instanceof BannerHomeFragment)) {
            PreferenceUtil.getInstance(MainActivity.this).setLastPage(R.id.action_home);
            getBottomNavigationView().setSelectedItemId(PreferenceUtil.getInstance(this).getLastPage());
    ...
    public boolean onCreateOptionsMenu(final Menu menu) {
        ...
        menu.removeItem(R.id.action_go_to_start_directory);
        if (isFolderPage()) {
    ...
    public void setCurrentFragment(@NonNull Fragment fragment, @NonNull String tag) {
        ...
        if (currentFragment == null || targetFragment != currentFragment) {
            Fragment lastFragment = (Fragment) currentFragment;
            if (targetFragment != null) {
                transaction.show(targetFragment);
                currentFragment = (MainActivityFragmentCallbacks) targetFragment;
            } else {
                transaction.add(R.id.fragment_container, fragment, tag);
                currentFragment = (MainActivityFragmentCallbacks) fragment;
            }
            if (lastFragment != null) {
                transaction.hide((Fragment) lastFragment);
            }
            transaction.commit();
        }
    }
    ...
    private Fragment getCurrentFragment() {
        if (currentFragment instanceof Fragment) return (Fragment)currentFragment;
        else return getSupportFragmentManager().findFragmentById(R.id.fragment_container);
    }
    ...
    private void handlePlaybackIntent(@Nullable Intent intent) {
        ...
        if (intent.getAction() != null && intent.getAction()
            ...
            if (MusicPlayerRemote.getShuffleMode() == MusicService.SHUFFLE_MODE_SHUFFLE) {
                MusicPlayerRemote.openAndShuffleQueue(this, ArrayListExKt.toCommonData(songs), true);
            } else {
                MusicPlayerRemote.openQueue(this, ArrayListExKt.toCommonData(songs), 0, true);
            }
            handled = true;
        }
        ...
        if (uri != null && uri.toString().length() > 0) {
            ...
            if (id >= 0) {
                ...
                ArrayList<CommonData> songs = PlaylistSongsLoader.INSTANCE.getPlaylistSongsFromDb(this, id);
                MusicPlayerRemote.openQueue(this, songs, position, true);
                handled = true;
            }
            ...
            if (id >= 0) {
                int position = intent.getIntExtra("position", 0);
                MusicPlayerRemote.openQueue(this, ArrayListExKt.toCommonData(AlbumLoader.getAlbum(this, id).getSongs()), position, true);
                handled = true;
            }
            ...
            if (id >= 0) {
                int position = intent.getIntExtra("position", 0);
                MusicPlayerRemote.openQueue(this, ArrayListExKt.toCommonData(ArtistLoader.getArtist(this, id).getSongs()), position, true);
                handled = true;
            }
        }
        if (handled) {
    ...
    private boolean isHomePage() {
        return getCurrentFragment() instanceof BannerHomeFragment;
    }
    ...
    private void selectedFragment(final int itemId) {
        ...
        switch (itemId) {
            ...
            case R.id.action_gift:
                inRewarding = true;
                setCurrentFragment(RewardFragment.newInstance(), RewardFragment.TAG);
                break;
