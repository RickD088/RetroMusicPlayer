...
class LyricsActivity : AbsMusicServiceActivity(), View.OnClickListener,
    ...
    private lateinit var song: CommonData
    private var lyricsString: String? = null

    private val googleSearchLrcUrl: String
        get() {
            var baseUrl = "http://www.google.com/search?"
            var query = song.getSongTitle() + "+" + song.getSongSinger()
    ...
    private fun updateTitleSong() {
        ...
        toolbar.title = song.getSongTitle()
        toolbar.subtitle = song.getSongSinger()
    }
    ...
    private fun showSyncedLyrics() {
        var content = ""
        try {
            content = LyricUtil.getStringFromFile(song.getSongTitle(), song.getSongSinger())
        } catch (e: Exception) {
            e.printStackTrace()
        }
                ...
                if (song.localSong()) {
                    ...
                    WriteTagsAsyncTask(this@LyricsActivity).execute(
                        WriteTagsAsyncTask.LoadingInfo(
                            getSongPaths(song.getLocalSong()), fieldKeyValueMap, null
                        )
                    )
                }
            }
            ...
            updateLyricsAsyncTask = object : AsyncTask<Void?, Void?, Lyrics?>() {
                override fun doInBackground(vararg params: Void?): Lyrics? {
                    ...
                    return if (TextUtils.isEmpty(data)) {
                        ...
                        if (song.localSong()) {
                            Lyrics.parse(song.getLocalSong(), data!!)
                        } else null
                    }
                }
    ...
    class SyncedLyricsFragment : BaseLyricsFragment(), MusicProgressViewUpdateHelper.Callback {
        ...
        private fun loadLRCLyrics() {
            ...
            val song = MusicPlayerRemote.currentSong
            if (LyricUtil.isLrcFileExist(song.getSongTitle(), song.getSongSinger())) {
                showLyricsLocal(LyricUtil.getLocalLyricFile(song.getSongTitle(), song.getSongSinger()))
            }
        }
